...
ap_fixed<16,2> randFW::DellYanGenerator::_lut_sintheta[1024];
ap_fixed<10,4>  randFW::DellYanGenerator::_lut_eta[1024];
ap_ufixed<10,3> randFW::DellYanGenerator::_lut_phi[256];
...
void randGen(bool status,ap_uint<16> *randNum)
{
    ...
    void _lutFill_eta(ap_fixed<10,4> _lut_eta[1024])
    {
        ...
        {
         #pragma HLS unroll
            //_lut_eta[i]= -log(min(abs(tan( acos(1.0-2.0*i/1024.0 + 1e-9)/2.0 )),1e3));
            _lut_eta[i]= -1.0*log(tan( acos(1.0-2.0*i/1024.0 + 1e-9)/2.0 ));
        }
    }
    ...
    void DellYanGenerator::init()
    {
        randomWord_16Bit wordgen; wordgen.init();
    ...
    ap_uint<64> DellYanGenerator::getDimuonPairs()
    {
           ...
           ap_uint<8> rid1=word & 0xff; word=word >>8;
           ap_uint<8> rid3=word & 0xff; word=word >>8;
           word=wordGen.getRandom();
           ap_uint<10> rid2=word & 0x3ff; word=word >>10;
...
extern "C" {
  ...
  void drellYanPairGenerator( ap_uint<32> MU1[N_DY_GEN] , ap_uint<32> MU2[N_DY_GEN])
  {
        ...
        {
            diMuons[i]=dyGen.getDimuonPairs() ; std::cout<<" DIMU P : "<<diMuons[i]<<"\n";
        }
        
        MU1[1] = wordGen.isInitialized ;  
        MU1[1] = wordGen.b3[0].current_state(15,0); 
        MU1[2] = wordGen.b3[1].current_state(15,0); 
